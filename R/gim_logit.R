bootstrapIM.logit <- function(lm1, B, B2, cluster=NA, time=NA)
{
    X <- model.matrix(lm1)
    y <- unname(lm1$y)
    ...
    for(i in 1:B){
        ...
        lm1B <- glm(yB ~ model.matrix(lm1)[,-1], family=binomial("logit"))
        ...
        for(j in 1:B2){
            ...
            lm1B2 <- glm(yB2 ~ model.matrix(lm1)[,-1], family=binomial("logit"))
